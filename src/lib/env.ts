...
export const envSchema = z.object({
  ...
  baseNetwork: z.custom<Chain>(
    (data) => data === baseSepolia || data === base,
    { message: "baseNetwork should be either base or baseSepolia" }
  ),
});

export type Env = z.infer<typeof envSchema>;
const baseNetwork = ENV.networkID === "mainnet" ? base : baseSepolia;
export const env: Env = envSchema.parse({ ...ENV, baseNetwork });
